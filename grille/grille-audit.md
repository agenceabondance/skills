...
Une ligne par contrôle. Chaque contrôle porte sa source (`[NN]` = fiche du corpus, `[USxxxx]` = brevet), son **niveau de preuve** (voir `references/00-index.md`) et dit si le propriétaire peut agir. **Pas de score global** : les poids sont inconnus ([04]), on rend un verdict par famille.
